...
import numpy as np
import torch.nn as nn
#from tools.attn import  OD_Attention
import torch.nn.functional as F
...
class TimeFeatureBlock(nn.Module):
    def __init__(self,args1, env_days):
        ...
        # Encoding
        self.enc_embedding = DataEmbedding(args1['enc_in'], d_model=128, embed_type='timeF', freq='d', dropout=0.05)
        # Attention
        Attn = ProbAttention
        # Encoder
        ...
        self.projection = nn.Linear(126, args1['c_out'], bias=True)
...
class GEFormer(nn.Module):
    def __init__(self, trial, args1, snp_len, env_days):
        super(GEFormer, self).__init__()
        
        dout = trial.suggest_float("dropout", args1['dropout_1'], args1['dropout_2'])
        dep = trial.suggest_int("depth", args1['depth_1'], args1['depth_2'])  
        L1 = trial.suggest_int("neurons1",args1['neurons1_1'],args1['neurons1_2'])  
        L2 = trial.suggest_int("neurons2",args1['neurons2_1'],args1['neurons2_2'])  
        ...
        self.TimeFeatureBlock = TimeFeatureBlock(args1, env_days)
...
#from mymodel_optuna import GEFormer
...
def op_train(arg11,arg22,arg33,arg44,arg55,arg66,arg77):
    ...
    df_data_raw = pd.read_csv(args1['env_path'])

    env_factor = df_data_raw.shape[1]
    env_days = df_data_raw.shape[0]

    args1['enc_in'] = env_factor-2
    args1['c_out'] = env_factor-2
    ...
    with open(args1['geno_path']) as file: 
    ...
    df = pd.read_csv(args1['pheno_path'])
    filtered_columns = [col for col in df.columns if col.startswith(args1['pheno_name']+"_")]
    ...
    def objective(trial):  
        ...
        batch = trial.suggest_int("batch",args1['batch_1'],args1['batch_2'])
        ...
        net = GEFormer(trial, args1, snp_len, env_days).to(args1['device'])
        lr = trial.suggest_float("lr", args1['lr_1'], args1['lr_2'], log=True)
        ...
        for epoch in range(100):
            ...
            for j,(id, phe, dictseq, data_x, data_stamp) in enumerate(train_loader, 0):
                ...
                dictseq = torch.stack(dictseq).to(args1['device'])
                phe = phe.clone().detach().to(args1['device'])  
                phe = phe.float()
                dictseq = dictseq.float()
                data_x = data_x.to(args1['device'])
                data_stamp = data_stamp.to(args1['device'])
                optimizer.zero_grad()
    ...
    study.optimize(objective,args1['optuna_eph'])  
