...
class GEFormer(nn.Module):
    def __init__(self, args, snp_len, env_days):
...
def predict(arg1, arg2, arg3, arg4, arg5, arg6):
    setup_seed(147)

    parser = argparse.ArgumentParser(description="Genome-wide prediction model for genotype-environment interaction.")

    parser.add_argument('--geno_path', type=str, default=arg1, help='path of genotype file')
    parser.add_argument('--env_path', type=str, default=arg2, help='path of environment file')
    parser.add_argument('--pheno_name', type=str, default=arg3, help='name of phenotype')
    parser.add_argument('--model_path', type=str, default=arg4, help='path of model file')
    parser.add_argument('--result_path', type=str, default=arg5, help='path of result file')
    parser.add_argument('--device', default=arg6, help='device id (i.e. 0 or 0,1 or cpu)')

    args = parser.parse_args()

    csvFile = open(args.result_path, 'w', newline='', encoding='utf-8')
    ...
    df_data_raw = pd.read_csv(args.env_path)
    env_factor = df_data_raw.shape[1]
    env_days = df_data_raw.shape[0]

    args.enc_in = env_factor-2
    args.c_out = env_factor-2
    ...
    header = ['ID', 'env', args.pheno_name]
    writer.writerow(header)
    list_phe = [negative_infinity for _ in range(len(list_number))]

    mdata_test = myDataset(list_number, list_phe, dictSeq, dictSeq_1)
    hh1 = int(len(mdata_test))
    ...
    net = torch.load(pthfile, map_location=args.device, weights_only=False)
    loss_func = nn.MSELoss()

    net.eval()

    for jj, (id, phe, dictseq, data_x, data_stamp) in enumerate(test_loader, 0):
        ...
        for i in range(len(id)):
            iname = '_'.join(id[i].split('_')[:-1])
            ename = id[i].split('_')[-1]
            writer.writerow([iname, ename, "{:.4f}".format(pred[i])])
